...
public class StoreHouseRefreshView extends View implements CanRefresh {
    ...
    public StoreHouseRefreshView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public StoreHouseRefreshView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initView() {
        ...
        mLineWidth = StoreHouseDisplay.getInstance(getContext()).dp2px(1);
        mDropHeight = StoreHouseDisplay.getInstance(getContext()).dp2px(80);
        mHorizontalRandomness = StoreHouseDisplay.getInstance(getContext()).SCREEN_WIDTH_PIXELS / 2;
    ...
    public StoreHouseRefreshView setLineWidth(int width) {
        mLineWidth = width;
    ...
    public StoreHouseRefreshView setTextColor(int color) {
        mTextColor = color;
        for (int i = 0; i < mItemList.size(); i++) {
    ...
    public StoreHouseRefreshView setDropHeight(int height) {
        mDropHeight = height;
        return this;
    }
    ...
    private int getTopOffset() {
        return getPaddingTop() + StoreHouseDisplay.getInstance(getContext()).dp2px(10);
    }

    private int getBottomOffset() {
        return getPaddingBottom() + StoreHouseDisplay.getInstance(getContext()).dp2px(10);
    }
    ...
    public void initWithPointList(ArrayList<float[]> pointList) {
        ...
        for (int i = 0; i < pointList.size(); i++) {
            ...
            PointF startPoint = new PointF(StoreHouseDisplay.getInstance(getContext()).dp2px(line[0]) * mScale, StoreHouseDisplay.getInstance(getContext()).dp2px(line[1]) * mScale);
            PointF endPoint = new PointF(StoreHouseDisplay.getInstance(getContext()).dp2px(line[2]) * mScale, StoreHouseDisplay.getInstance(getContext()).dp2px(line[3]) * mScale);
    ...
    public void onPositionChange(float currentPercent) {
         currentPercent = Math.min(1f, currentPercent/3);
